...
void display(Node *ptr, int level)
{
    if (ptr)
    {
        ...
        if (ptr->isLeaf)
        {
            // Print the required number of spaces based on the level
            for (i = 0; i < level; i++)
                printf("   ");
        ...
        {
            // Recursively display child nodes with increased level
            for (i = 0; i < ptr->numKeys; i++) {
                display(ptr->ptrs.childNodes[i], level + 1);

                // Print the parent node's key
                for (int j = 0; j < level; j++)
                    printf("   ");
                printf("%d\n", ptr->keys[i]);
            }

            // Display the last child
            display(ptr->ptrs.childNodes[ptr->numKeys], level + 1);
        }
    }
}
